...
void dl32String::HexLongCasting(long Number)
{
	...
	Array=new char[11];//Lo anterior m�s el caracter final (Cadena C-style)
	int delta,realdelta;
	...
	{
		delta=int(Number)>>(i*4) & 0x0000000F;//delta es el numero (en decimal) de �sta posici�n
		if(delta<=9)
			realdelta=delta;
		else
			realdelta=delta+7;// El caracter 'A' no est� despu�s del '9' (Ver tabla ASCII)
