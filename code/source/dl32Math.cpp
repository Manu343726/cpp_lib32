...
{
	//La gran mayor�a lo entiendo, pero me gustar�a saber de donde se sac� el n�mero m�gico.....
	long i;
	float x2, y;
...
dl32Point2D dl32Line2D::Intersection(dl32Line2D L1, dl32Line2D L2,bool externalParallelCheck) throw(dl32InfiniteIntersectionException)
{
	...
	{
		//Resoluci�n del sistema de ecuaciones formado por las ecuaciones parametricas de las rectas:
...
dl32OBB2D::dl32OBB2D(dl32Point2D pointCloud[],int pointCount)
{
	...
	dl32Vector2D aux;//Vector auxiliar para la b�squeda del eje x local (En coordenadas de mundo)
	dl32Vector2D xAxis;//El eje x local en coordenadas de mundo

	//FASE 1: Encontrar el eje x �ptimo, en coordenadas de mundo, para el OBB que envuelve la nube:
	///////////////////////////////////////////////////////////////////////////////////////////////

	/******************************************************************************************************************************************
	* �ste m�todo consiste en obtener el eje x como el sumatorio de los radiovectores de todos los v�rtices respecto al baricentro de la nube.*
	* El problema es que si se suman los radiovectores, seguramente se anulen unos con otros, ya que muchos tendr�n sentidos opuestos.		  *
	* Como lo que nos interesa es la direcci�n, no el sentido, sumamos los vectores siempre en la misma direcci�n, es decir, si el sumatorio  *
	...
	******************************************************************************************************************************************/
	...
	/***************************************************************************************************************************************
	* La transformaci�n de local a mundo (_toWorld) es la concatenaci�n de la rotaci�n que lleva el eje x del mundo al eje x local (xAxis) *
	* m�s una traslaci�n que lleve el origen de coordenadas al centro de la nube (cloudCenter). 										   *
	* Por supuesto, la transformaci�n de mundo a local (_toLocal) es la transformaci�n inversa.											   *
	***************************************************************************************************************************************/

	//NOTA: Como el eje x es unitario, el seno del �ngulo de rotaci�n es la coordenada y del eje, y el coseno la coordenada x:
...
{
	_toLocal=_toLocal+transformation;//Se encadenan las dos transformaciones (Premultiplica la actual por la transformaci�n)
	_toWorld=_toWorld*transformation;//Multiplicar la actual por la transformaci�n es equivalente a premultiplicar por la inversa de la transformaci�n
}
...
bool dl32OBB2D::Collide(dl32OBB2D O1, dl32OBB2D O2)
{
	...
	/*********************************************************************************************************************************************************
	* El m�todo consiste en testear la colisi�n a trav�s de los AABBs correspondientes a los dos OBBs en sus coordenadas locales.                            *
	* Para ello, se hacen dos test de colisi�n, uno en las coordenadas locales de un OBB, y otro en las coordenadas locales del otro OBB.                    *
	* Si ambos test detectan colisi�n, los OBBs colisionan.																									 *                                                                                                                               *
