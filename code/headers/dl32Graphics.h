...
private:
	...
	dl32GraphicsClass(const dl32GraphicsClass&); //As� evitamos que se realicen copias
	dl32GraphicsClass& operator=(const dl32GraphicsClass&);//As� evitamos que se hagan asignaciones
protected:
	LPDIRECT3D9 _d3d;	
...
public:
	...
	dl32GraphicsClass(HWND hwnd,int Width,int Height,
					  ...
					  throw(dl32Direct3DInitFailedException) : _lastObject(_mockObject) {_setup(hwnd,Width,Height,Windowed,tripleBuffer,vSync,refreshRate);} //ERROR: NO DEBER�A SER INLINE 

	dl32GraphicsClass(dl32Window* window ,
					  bool Windowed=DL32DEFAULTS_GRAPHICS_WINDOWED, bool tripleBuffer = DL32DEFAULTS_GRAPHCS_TRIPLEBUFFER,
					  bool vSync = DL32DEFAULTS_GRAPHICS_VSYNC, int refreshRate = DL32DEFAULTS_GRAPHICS_REFRESHRATE)
					  throw(dl32Direct3DInitFailedException) : _lastObject(_mockObject) {_lastObject = DL32BUFFEROBJECT();_setup(window->GetHwnd(),window->GetWidth(),window->GetHeight(),Windowed,tripleBuffer,vSync,refreshRate);} //ERROR: NO DEBER�A SER INLINE 
